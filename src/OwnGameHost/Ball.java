...
public class Ball extends JLabel implements Runnable {
	...
	public Ball(int startX, int maxWidth, int maxHeight,  Bar[] bars, Wall[] walls, JPanel mainPnl) {
		...
		this.xSpeed = 0;
		this.ySpeed = 7;//rd.nextInt(MAXSPEED - 5) + 5;
	}
	...
	public void run() {
		...
		while(true) {
			...
			try {
				TcpClient.sendMsg(bars[0].getX() + "," + -1 + "," + -1 + "," + getX() + "," + getY() + "," + -1);
...
//		if(bars[1].getBounds().contains(getBounds().getCenterX(), getBounds().getMaxY())) {
//			ySpeed = -Math.abs(ySpeed);
//			
//			xSpeed += bars[1].moveRightAmt;
//			xSpeed -= bars[1].moveLeftAmt;
//			if(xSpeed > MAXSPEED) xSpeed = MAXSPEED;
//			if(xSpeed < (-1) * MAXSPEED) xSpeed = (-1) * MAXSPEED;
//			
//			bars[1].resetMovLeftAmt();
//			bars[1].resetMovRightAmt();
//		}
//	}
	...
	void checkBumpedWall() {
		...
		for(int i = 0; i < walls.length; i++) {
			...
			if(isBumped) {
				walls[i].scoring();
				try {
					TcpClient.sendMsg(bars[0].getX() + "," + -1 + "," + -1 + "," + getX() + "," + getY() + "," + i);
				} catch(NullPointerException e) {}
				crash(i);
			}
		}
