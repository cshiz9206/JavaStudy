...
public class BreakOutFrame extends JFrame implements KeyListener, ActionListener, MouseListener {
	...
	void createMainPnl(int playerCnt) {
		...
		for(int i = 0; i < playerCnt; i++) {
			bars[i] = new Bar(0, frameWidth - 20, frameHeight, "..\\BreakOut_figure\\user" + i + ".png");
			mainPnl.add(bars[i]);
		}
//		
...
//		mainPnl.add(bar2);
		...
		ball = new Ball(0, frameWidth - 20, frameHeight, bars,  walls, mainPnl);
		Ball.isDead = false;
		mainPnl.add(ball);
...
//		mainPnl.add(info);
	...
	void createPSpinner() {
		playerSetSpnr = new JSpinner(new SpinnerNumberModel(1, 1, 2, 1));
...
//	}
	...
	void process() {
		...
		while(true) {
			if(playerSetSpnr.getValue().equals(1)) init1PGame();
			else if(playerSetSpnr.getValue().equals(2)) init2PGame();
	...
	void init2PGame() {
		...
		// create connection
		tcpConnection = new TcpClient(bars[1], ball, getX(), getY());
		tcpConnection.start();
